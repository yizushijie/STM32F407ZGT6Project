...
extern "C" {
	...
	//===结构体定义
	typedef struct _OLED_IIC_HandleType			OLED_IIC_HandleType;

	//===指针结构体定义
	typedef struct _OLED_IIC_HandleType			*pOLED_IIC_HandleType;

	//===OLED的数据结构体
	struct _OLED_IIC_HandleType
	{
		GPIO_HandleType		msgRST;																						//---复位端口
	...
	//===外部调用接口
	extern OLED_IIC_HandleType						g_OLEDI2CDevice0;
	extern pOLED_IIC_HandleType					pOLEDI2CDevice0;
