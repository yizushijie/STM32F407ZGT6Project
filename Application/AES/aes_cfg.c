...
void increment_iv(UINT8_T iv[], int counter_size)
{
	...
	// Use counter_size UINT8_Ts at the end of the IV as the big-endian integer to increment.
...
void aes_encrypt_ctr(const UINT8_T in[], size_t in_len, UINT8_T out[], const UINT32_T key[], int keysize, const UINT8_T iv[])
{
	...
	aes_encrypt(iv_buf, out_buf, key, keysize);
	xor_buf(out_buf, &out[idx], in_len - idx);   // Use the Most Significant UINT8_Ts.
}
...
//////说		明：
//////////////////////////////////////////////////////////////////////////////
UINT32_T SubUINT32_T(UINT32_T UINT32_T)
{
	unsigned int result;
...
void aes_key_setup(const UINT8_T key[], UINT32_T w[], int keysize)
{
	...
	for (idx = Nk; idx < Nb * (Nr + 1); ++idx)
	{
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
			temp = SubUINT32_T(KE_ROTUINT32_T(temp)) ^ Rcon[(idx - 1) / Nk];
		else if (Nk > 6 && (idx % Nk) == 4)
			temp = SubUINT32_T(temp);
		w[idx] = w[idx - Nk] ^ temp;
	}
}
...
//////说		明：
//////////////////////////////////////////////////////////////////////////////
void SubUINT8_Ts(UINT8_T state[][4])
{
...
//////说		明：
//////////////////////////////////////////////////////////////////////////////
void InvSubUINT8_Ts(UINT8_T state[][4])
{
...
void aes_encrypt(const UINT8_T in[], UINT8_T out[], const UINT32_T key[], int keysize)
{
	...
	// Copy input array (should be 16 UINT8_Ts long) to a matrix (sequential UINT8_Ts are ordered
	...
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[4]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[8]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[12]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[16]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[20]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[24]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[28]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[32]);
	SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[36]);
	if (keysize != 128)
	{
		SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[40]);
		SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[44]);
		if (keysize != 192)
		{
			SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[48]);
			SubUINT8_Ts(state); ShiftRows(state); MixColumns(state); AddRoundKey(state, &key[52]);
			SubUINT8_Ts(state); ShiftRows(state); AddRoundKey(state, &key[56]);
		}
		else
		{
			SubUINT8_Ts(state); ShiftRows(state); AddRoundKey(state, &key[48]);
		}
	}
	else
	{
		SubUINT8_Ts(state); ShiftRows(state); AddRoundKey(state, &key[40]);
	}
...
void aes_decrypt(const UINT8_T in[], UINT8_T out[], const UINT32_T key[], int keysize)
{
	...
	if (keysize > 128)
	{
		if (keysize > 192)
		{
			...
			InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[52]); InvMixColumns(state);
			InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[48]); InvMixColumns(state);
		}
		else
		{
			AddRoundKey(state, &key[48]);
		}
		InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[44]); InvMixColumns(state);
		InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[40]); InvMixColumns(state);
	}
	else
	{
		AddRoundKey(state, &key[40]);
	}
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[36]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[32]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[28]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[24]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[20]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[16]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[12]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[8]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[4]); InvMixColumns(state);
	InvShiftRows(state); InvSubUINT8_Ts(state); AddRoundKey(state, &key[0]);
...
//////说		明：
//////////////////////////////////////////////////////////////////////////////
unsigned char*encrypt(const unsigned char* data, int in_len, unsigned int* out_len,	const unsigned char* key, const unsigned char* iv)
{
...
//////说		明：
//////////////////////////////////////////////////////////////////////////////
unsigned char * decrypt(const unsigned char* data, int in_len, unsigned int* out_len,const unsigned char* key, const unsigned char* iv)
{
	if (in_len <= 0 || in_len >= MAX_LEN)
	{
		return NULL;
